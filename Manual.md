...
* **画板模式（黑/白板模式）**
    ...
    *  支持添加新页面和页面切换
    *  支持直尺、三角尺、量角器等几何尺具（同一种可同时开多把），可用于课堂画线、辅助读角和定点标记
    *  支持多指书写：黑板模式界面左下角人像图标为切换按钮
    *  支持展台抓拍：工具栏相机图标打开展台窗口，抓拍的照片直接贴到画布上，可拖动、缩放、旋转、删除；照片左侧的裁剪按钮可裁掉多余部分，进入后拖八个把手调整裁剪范围、拖框内挪动位置，右上角确认、左上角取消
    *  支持拖动黑板：黑板模式界面左下角的十字箭头图标可在“书写”和“拖动”之间切换。进入拖动状态后，单指或笔在屏幕上拖动即可把整块黑板挪走——板书墨迹、展台抓拍的照片（连同它的选中框与手柄）、直尺三角尺量角器等尺具都挂在同一个位移上一起走，相对位置不变。板书区有 9 倍屏幕那么大，上下左右各能挪 4 个屏幕；整块屏幕任何时候都能书写，不会出现“看得见却画不上、笔尖变箭头”的死角。拖到别处之后新抓拍的照片、新打开的尺具仍旧落在当前可见区正中。切到屏幕模式再切回来，黑板还停在原处。再次点击图标回到书写状态
	*  默认是黑板，可以在设置中调成白板
* **屏幕画笔模式**
...
### 几何尺具
* **打开尺具**：进入黑板/白板模式后，在浮动工具栏点击“尺”，可选择直尺、三角尺或量角器。**每点一次就新开一把**，同一种尺具可以同时开好几把——例如两把直尺摆成夹角，沿两把尺子各画一条线，画的时候尺子不用挪。每把都有各自的位置、角度和长短，点某一把就把它提到最上层。
* **直尺**：拖动直尺中部可移动位置，拖动旋转点旁的旋转按钮可调整角度，拖动右侧手柄可调整长度（只保留下限，**长度不限**，一直拉长都可以）；沿直尺上下边缘拖动可辅助绘制平行于尺边的直线，尺身中线可配合旋转点校准平行方向。尺子比屏幕还长、右端把柄够不着时，把光标放在尺身上滚动滚轮就能就地旋转（按住 Shift 每格 1°，不按每格 0.1°）。
* **三角尺**：可拖动、旋转，并可通过两个直角边端点手柄调整边长（同样不限长）；沿三角尺边缘拖动可辅助画线。两条直角边刻度从直角顶点外 5mm 起始，并在非直角顶点前 1cm 结束。滚轮同样可以旋转。
* **量角器**：可拖动、旋转、缩放；点击刻度弧附近可吸附到最近整度并留下定点标记。定点标记归这把量角器所有：点 X 关掉量角器，它的标记跟着一起消失（别的量角器的标记不受影响），点“清屏”也会把它们清掉。
* **黑白板适配**：尺具刻度会随黑板或白板背景自动切换明暗颜色，保持可读性。
### ppt放映模式
