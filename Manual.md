...
* **画板模式（黑/白板模式）**
    ...
    *  支持添加新页面和页面切换
    *  支持直尺、三角尺、量角器等几何尺具，可用于课堂画线、辅助读角和定点标记
    ...
    *  支持拖动黑板：黑板模式界面左下角的十字箭头图标可在“书写”和“拖动”之间切换。进入拖动状态后，单指或笔在屏幕上拖动即可把整块黑板挪走——板书墨迹、展台抓拍的照片（连同它的选中框与手柄）、直尺三角尺量角器等尺具都挂在同一个位移上一起走，相对位置不变。展台照片放大到超过屏幕时，可以拖动黑板平移到想看的区域；写着写着需要腾地方，也可以整体挪开继续写。再次点击图标回到书写状态
	*  默认是黑板，可以在设置中调成白板
* **屏幕画笔模式**
...
### 几何尺具
* **打开尺具**：进入黑板/白板模式后，在浮动工具栏点击“尺”，可选择直尺、三角尺或量角器。
* **直尺**：拖动直尺中部可移动位置，拖动旋转点旁的旋转按钮可调整角度，拖动右侧手柄可调整长度；沿直尺上下边缘拖动可辅助绘制平行于尺边的直线，尺身中线可配合旋转点校准平行方向。
* **三角尺**：可拖动、旋转，并可通过两个直角边端点手柄调整边长；沿三角尺边缘拖动可辅助画线。两条直角边刻度从直角顶点外 5mm 起始，并在非直角顶点前 1cm 结束。
* **量角器**：可拖动、旋转、缩放；点击刻度弧附近可吸附到最近整度并留下定点标记；点击 X 可隐藏量角器。
