...
- **黑板拖动**：黑板模式左下角新增拖动状态开关，与书写状态互切。进入后单指或笔拖拽即可挪动整块黑板——板书、展台照片（连同手柄）、几何尺具挂在同一位移上整体走，相对位置不变。板书区是黑板画布的 9 倍屏幕大小，上下左右各可挪 4 个屏幕，整块屏幕始终都能书写（不会出现画不上字、笔尖变箭头的死角）；拖过之后新抓拍的照片和尺具仍落在当前可见区正中
- **几何工具**：黑板/白板模式可用直尺、三角尺、量角器，支持拖动、旋转、缩放，自动切换刻度颜色；直尺和三角尺支持贴边辅助画线，长度不设上限（只保留下限），尺子比屏幕还长时可滚动滚轮就地旋转；量角器支持点击刻度弧定点标记。**同一种尺具可以同时开多把**：工具栏点一次就新开一把、错开落位，画平行线等场景可同时摆两把；每把独立关闭，量角器的定点标记跟着它一起消失
